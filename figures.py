...
def plot_FD(axes, sf_scheme, dt=60., N=1680,
    ...
    # axes[2].semilogy(dt*np.array(range(len(ustar))), ustar, "+", label=r"$u_\star^\text{FD}$")
...
def fig_integration_1dekman():
    ...
    sf_scheme_FV = "FV2"
    # plot_FV(axes, sf_scheme_FV, delta_sl=z_levels[1],
    #         N=N, dt=dt, z_levels=z_levels)
    # plot_FV(axes, "FV2 free", delta_sl=z_levels[1],
    #         N=N, dt=dt, z_levels=z_levels)
