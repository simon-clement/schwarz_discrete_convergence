...
def linear_test_large_window():
    ...
    for M in (6, 11):
        ...
        total_interface = np.array([LAMBDA*u_real(size_domain, n*dt) + nu*phi_real(x=size_domain-h/2, t=n*dt)
                for n in range(N+1)])
        def forcing(t):
...
def sinus_test_large_window_kc1():
    ...
    for M in (20, 41):
        ...
        LAMBDA=1.
        N = int(T/dt)
        ocean_integrator = OceanPadeFD(r=r, nu=nu, M=M, SIZE_DOMAIN=size_domain, LAMBDA=LAMBDA, DT=dt, K_c=1)
        h = ocean_integrator.h
        ...
        total_interface = np.array([LAMBDA*u_real(size_domain, n*dt) + nu*phi_real(x=size_domain, t=n*dt)
                for n in range(N+1)])
        def forcing(t):
            return np.flipud(forcing_real(x_u, t))
